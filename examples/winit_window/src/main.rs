...
impl Display for PixelBuffer<'_> {
    fn plot(&mut self, x: i64, y: i64, color: &drawing::Color) {
        ...
        self.screen[(x + y * self.width) as usize] = color.2 as u32 | ((color.1 as u32) << 8) | ((color.0 as u32) << 16);
    }
}
    ...
    event_loop.run(move |event, elwt| {
        ...
        match event {
            Event::WindowEvent { window_id, event: WindowEvent::RedrawRequested } if window_id == window.id() => {
                ...
                let mut buffer = PixelBuffer { screen: surface.buffer_mut().unwrap(), height: height as i64, width: width as i64};
